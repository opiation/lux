    ...
    configureServer(server) {
      ...
      server.watcher.on("change", async (path) => {
        ...
        try {
          ...
          const updateMessage = `Dependencies installed after changes to ${Path.basename(path)}`;
      ...
      const listeningMessage = `  ${Chalk.grey("➜")}  ${Chalk.yellow.dim(name + ":")} ${Chalk.grey(
        "Auto-installing on changes to package.json files..."
      )}`;
